...
class GlobalMetricService:
    ...
    async def update_or_create_metric(self, metric: PostMetricSchema) -> GlobalMetric:
        ...
        if today_metric:
            updated_values = self._increment_metric(
                existing_metric=today_metric, new_metric=metric
            )
        ...
        # Если метрики за сегодня нет, используем последнюю метрику для накопительных значений
            ...
            if last_metric:
                new_values = self._initialize_metric_with_previous(
    ...
    def _increment_metric(
        self, existing_metric: GlobalMetric, new_metric: PostMetricSchema
    ) -> dict:
        """
        ...
        alltime_learned_percents = self._calculate_percentage(
            part=alltime_learned_amount, whole=alltime_userwords_amount
        )
        learned_percents = self._calculate_percentage(
            part=learned_amount, whole=alltime_userwords_amount
        )

        return {
            "alltime_words_amount": existing_metric.alltime_words_amount
            + new_metric.add_words_amount,
    ...
    def _initialize_metric_with_previous(
        self, last_metric: GlobalMetric, new_metric: PostMetricSchema
    ) -> dict:
        """
        ...
        learned_percents = self._calculate_percentage(
            part=new_metric.learned_amount, whole=new_metric.add_userwords_amount
        )

        return {
            "alltime_words_amount": last_metric.alltime_words_amount
            + new_metric.add_words_amount,
            "alltime_userwords_amount": alltime_userwords_amount,
            "alltime_learned_amount": alltime_learned_amount,
            "alltime_learned_percents": alltime_learned_percents,
    ...
    async def get_metric(
        ...
        metrics: list[GlobalMetric] = await self.repo.get_all_by_filter(
            filters=tuple(filters), order=GlobalMetric.id.asc(), limit=0
        )
